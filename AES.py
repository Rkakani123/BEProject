...
class AES:
    ...
    def substituteBytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = AC.Saltbox[s[i][j]]

    def inverseSubstituteBytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = AC.InvSaltbox[s[i][j]]
    ...
    def shiftKey(self, key):
        ...
        for i in range(4, 4 * 11):
            ...
            if i % 4 == 0:
                newKey = self.roundKey[i-4][0]^AC.Saltbox[self.roundKey[i-1][1]]^AC.Rcon[i//4]
                self.roundKey[i].append(newKey)

                for j in range(1, 4):
                    newKey = self.roundKey[i-4][j]^AC.Saltbox[self.roundKey[i-1][(j+1)%4]]
                    self.roundKey[i].append(newKey)
            else:
                for j in range(4):
    ...
    def decryptData(self, encrypted_chunks):
        data=""
